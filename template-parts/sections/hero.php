...
$video_url     = (string) isg_acf_value('hero_video_url', $default_video, get_the_ID());
$lightbox_url  = (string) isg_acf_value('hero_lightbox_url', $video_url, get_the_ID());
$poster_url    = isg_acf_image_url('hero_video_poster', (string) get_the_ID(), isg_asset_uri('img/img-test-1.jpg'));

$title        = (string) isg_acf_value('hero_title', "Large-Diameter\nSpiral-Welded Pipes", get_the_ID());
$subtitle     = (string) isg_acf_value('hero_subtitle', 'Infrastructure & industrial projects', get_the_ID());
$catalog_text = (string) isg_acf_value('hero_catalog_label', 'Download catalogue', get_the_ID());
$catalog_url  = (string) isg_acf_value('hero_catalog_url', '#', get_the_ID());
$watch_text   = (string) isg_acf_value('hero_watch_label', 'Watch video', get_the_ID());
?>
	...
	<div class="isg-hero__conteiner container">
		<div class="isg-hero__content">
			<div class="isg-title-group">
				<h1 class="isg-display isg-text-center isg-hero__title"><?php echo wp_kses_post(nl2br(esc_html($title))); ?></h1>
			...
			<div class="isg-btn-group isg-btn-group--spread isg-btn-group--hero-actions">
				<a class="isg-btn isg-btn--blur" href="<?php echo esc_url($catalog_url); ?>"><?php echo esc_html($catalog_text); ?></a>
				<button
					type="button"
					class="isg-hero-watch-btn"
					data-isg-lightbox="<?php echo esc_attr($lightbox_url); ?>"
					data-isg-lightbox-kind="video"
